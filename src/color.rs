...
///
/// Uses "loosely packed" format where each color component occupies the
/// upper 6 bits of each byte: `[RRRRRR00, GGGGGG00, BBBBBB00]`.
///
...
impl ColorConfig for Rgb666Mode {
    ...
    fn write_pixel(color: Self::Color, buf: &mut [u8]) {
        let rgb = color.into_storage();
        // Extract 6-bit components and place in upper 6 bits of each byte
        buf[0] = (((rgb >> 12) & 0x3F) << 2) as u8;
        buf[1] = (((rgb >> 6) & 0x3F) << 2) as u8;
        buf[2] = ((rgb & 0x3F) << 2) as u8;
    }
